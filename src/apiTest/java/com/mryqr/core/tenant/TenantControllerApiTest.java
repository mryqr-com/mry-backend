...
class TenantControllerApiTest extends BaseApiTest {
    ...
    public void should_fail_update_subdomain_if_already_exists() {
        ...
        LoginResponse anotherResponse = setupApi.registerWithLogin(rMobile(), rPassword());
        Tenant anotherTenant = tenantRepository.byId(anotherResponse.getTenantId());
        setupApi.updateTenantPlan(anotherTenant, anotherTenant.currentPlan().withCustomSubdomainAllowed(true));
        UpdateTenantSubdomainCommand command = UpdateTenantSubdomainCommand.builder().subdomainPrefix(subdomainPrefix).build();
        assertError(() -> TenantApi.updateSubdomainRaw(anotherResponse.getJwt(), command), TENANT_WITH_SUBDOMAIN_PREFIX_ALREADY_EXISTS);
    }
    ...
    public void should_fetch_tenant_public_profile() {
        ...
        UploadedFile loginBackground = rImageFile();
        TenantApi.updateSubdomain(response.getJwt(), UpdateTenantSubdomainCommand.builder().subdomainPrefix(subdomainPrefix).build());
        TenantApi.updateBaseSetting(response.getJwt(),
                UpdateTenantBaseSettingCommand.builder().name("aTenantName").loginBackground(loginBackground).build());
