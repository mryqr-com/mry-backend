...
import static com.mryqr.core.submission.SubmissionUtils.*;
...
class SubmissionControllerApiTest extends BaseApiTest {
    ...
    public void submitter_should_fail_update_own_submission_if_update_not_allowed() {
        ...
        assertError(() -> SubmissionApi.updateSubmissionRaw(memberResponse.getJwt(), submissionId, updateSubmissionCommand),
                PAGE_NOT_ALLOW_CHANGE_BY_SUBMITTER);
    }
    ...
    public void submitter_should_fail_update_submission_once_approved() {
        ...
        assertError(() -> SubmissionApi.updateSubmissionRaw(memberResponse.getJwt(), submissionId, updateSubmissionCommand),
                CANNOT_UPDATE_APPROVED_SUBMISSION);
    }
    ...
    public void should_list_submissions_with_filters() {
        ...
        ListSubmissionsQuery command = ListSubmissionsQuery.builder()
                ...
                .filterables(Map.of(
                        radioControl.getId(),
                        newHashSet(firstRadioAnswer.getOptionId(), secondRadioAnswer.getOptionId(), thirdRadioAnswer.getOptionId()),
                        checkboxControl.getId(), newHashSet(firstCheckboxAnswer.getOptionIds().get(0), secondCheckboxAnswer.getOptionIds().get(0),
                                fourthCheckboxAnswer.getOptionIds().get(0))))
                .pageIndex(1)
                .pageSize(20)
                .build();
    ...
    public void should_export_submissions_to_excel() {
        ...
        assertEquals(itemCountAnswer.getItems().stream()
                ...
                .collect(Collectors.joining(", ")), record.get(19));
    ...
    public void should_fetch_submission_with_only_viewable_answers() {
        ...
        FSingleLineTextControl onlyViewableControl = defaultSingleLineTextControlBuilder().permissionEnabled(true).permission(CAN_MANAGE_APP)
                .submitterViewable(true).build();
    ...
    public void should_fetch_listed_submission_with_only_viewable_answers() {
        ...
        assertEquals(permissionedAnswer.getContent(),
                ((TextDisplayValue) listSubmission1.getDisplayAnswers().values().stream().findAny().get()).getText());
        ...
        assertEquals(updatedPermissionedAnswer.getContent(),
                ((TextDisplayValue) listSubmission2.getDisplayAnswers().get(updatedPermissionedAnswer.getControlId())).getText());
        assertEquals(onlyViewableAnswer.getContent(),
                ((TextDisplayValue) listSubmission2.getDisplayAnswers().get(onlyViewableAnswer.getControlId())).getText());
    }
    ...
    public void should_fetch_my_last_submission_for_auto_fill() {
        PreparedQrResponse response = setupApi.registerWithQr();
        FSingleLineTextControl autoFillControl = defaultSingleLineTextControlBuilder().fillableSetting(
                ControlFillableSetting.builder().autoFill(true).build()).build();
        FSingleLineTextControl nonAutoFillControl = defaultSingleLineTextControlBuilder().fillableSetting(
                ControlFillableSetting.builder().autoFill(false).build()).build();
