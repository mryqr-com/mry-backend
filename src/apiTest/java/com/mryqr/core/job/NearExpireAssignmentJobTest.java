...
public class NearExpireAssignmentJobTest extends BaseApiTest {
    ...
    @Test
    public void should_near_notify_assignments() {
        ...
        DateTime expireDateTime = DateTime.builder().date(expireTime.toLocalDate().toString()).time(ofPattern("HH:mm").format(expireTime)).build();
        DateTime nearExpireDateTime = DateTime.builder().date(nearExpireTime.toLocalDate().toString()).time(ofPattern("HH:mm").format(nearExpireTime)).build();
        ...
        assertEquals(assignment.getId(), FakeNotificationService.id);
    }
}
