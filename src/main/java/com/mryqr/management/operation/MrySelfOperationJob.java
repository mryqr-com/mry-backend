...
public class MrySelfOperationJob {
    ...
    private void syncOverallStatistics() {
        ...
        qrRepository.byCustomIdOptional(MRY_OPERATION_APP_ID, OPERATION_QR_CUSTOM_ID).ifPresent(qr -> {
            appRepository.cachedByIdOptional(MRY_OPERATION_APP_ID).ifPresent(app -> {
                try {
    ...
    private void deltaStatistics(QR qr, App app) {
        ...
        Set<Answer> deltaAnswers = buildDeltaAnswersForYesterday(page);
    ...
    private Set<Answer> buildDeltaAnswersForYesterday(Page page) {
        ...
        Query activeTenantCountQuery = Query.query(where(recentAccessedAt).gt(now().minus(1, DAYS)).lt(now()));
        FNumberInputControl activeTenantCountControl = (FNumberInputControl) allControls.get(ACTIVE_TENANT_CONTROL_ID);
        NumberInputAnswer activeTenantCountAnswer = NumberInputAnswer.answerBuilder(requireNonNull(activeTenantCountControl))
                .number((double) mongoTemplate.count(activeTenantCountQuery, Tenant.class))
                .build();
        ...
        return Set.of(deltaTenantAnswer, deltaAppAnswer, deltaQrAnswer, deltaSubmissionAnswer,
                deltaMemberAnswer, activeTenantCountAnswer, deltaDateAnswer);
    }

    private void totalStatistics(QR qr, App app) {
        Page page = app.pageById(SYNC_TOTAL_PAGE_ID);
        Set<Answer> totalAnswers = buildTotalAnswersForYesterday(page);
    ...
    private Set<Answer> buildTotalAnswersForYesterday(Page page) {
        LocalDate yesterday = LocalDate.now().minusDays(1);
        Instant yesterdayEnd = LocalDate.now().atStartOfDay(systemDefault()).toInstant();
        Query query = Query.query(where("createdAt").lt(yesterdayEnd));
        ...
        Platform platform = platformRepository.getPlatform();

        FNumberInputControl mobileAccessRatioControl = (FNumberInputControl) allControls.get(MOBILE_ACCESS_RATIO_CONTROL_ID);
        NumberInputAnswer mobileAccessRatioAnswer = NumberInputAnswer.answerBuilder(requireNonNull(mobileAccessRatioControl))
                .number((double) platform.mobileAccessRatio())
                .build();

        FDateControl totalDateControl = (FDateControl) allControls.get(TOTAL_DATE_CONTROL_ID);
        DateAnswer totalDateAnswer = DateAnswer.answerBuilder(requireNonNull(totalDateControl))
                .date(yesterday.toString())
                .build();

        return Set.of(totalTenantAnswer, totalAppAnswer, totalQrAnswer,
                totalSubmissionAnswer, totalMemberAnswer, mobileAccessRatioAnswer,
                totalDateAnswer);
    }
