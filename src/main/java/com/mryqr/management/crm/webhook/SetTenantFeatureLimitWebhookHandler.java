...
public class SetTenantFeatureLimitWebhookHandler implements TenantWebhookHandler {
    ...
    public void handle(BaseSubmissionWebhookPayload payload) {
        ...
        RadioAnswer assignmentAllowedAnswer = (RadioAnswer) answers.get(ASSIGNMENT_ALLOWED_CONTROL_ID);
        if (assignmentAllowedAnswer != null) {
            plan = plan.withAssignmentAllowed(
                    Objects.equals(assignmentAllowedAnswer.getOptionId(), ASSIGNMENT_ALLOWED_ACTIVATED_ID));
        }

        tenantCommandService.updateTenantPlan(tenantId, plan, NO_USER);
