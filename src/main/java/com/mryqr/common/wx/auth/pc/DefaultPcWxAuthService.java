...
public class DefaultPcWxAuthService implements PcWxAuthService {
    ...
    public PcWxAuthAccessTokenInfo fetchAccessToken(String code) {
        ...
        if (isBlank(openId) || isBlank(unionId) || isBlank(accessToken)) {
            throw new MryException(SYSTEM_ERROR, "Failed to get pc wx web auth access token.", mapOf("response", resultString));
        }

        //每次获取access token时，保存到redis中，便于首次登录绑定微信时获取昵称和头像
        stringRedisTemplate.opsForValue().set(PC_WX_AUTH_ACCESS_TOKEN_PREFIX + unionId, accessToken, ofHours(1));

        return PcWxAuthAccessTokenInfo.builder()
