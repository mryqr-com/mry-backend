...
public class InstanceSubmitCountNumberReporter implements NumberReporter {
    private final MongoTemplate mongoTemplate;

    @Override
    public boolean supports(NumberReport report) {
        if (report instanceof InstanceNumberReport instanceNumberReport) {
            return instanceNumberReport.getInstanceNumberReportType() == INSTANCE_SUBMIT_COUNT;
        }
        return false;
    }
    ...
    public Double report(NumberReport report, Set<String> groupIds, App app) {
        ...
        timeRangeOf(report.getRange()).ifPresent(timeRange -> criteria.and("createdAt").gte(timeRange.getStartAt()).lt(timeRange.getEndAt()));
