...
public class Tenant extends AggregateRoot {
    ...
    public void updateSubdomain(String newSubdomainPrefix, User user) {
        ...
        if (!Objects.equals(oldSubdomainPrefix, newSubdomainPrefix)) {
            ...
            raiseEvent(new TenantSubdomainUpdatedEvent(this.getId(), oldSubdomainPrefix, newSubdomainPrefix, user));
    ...
    public void onAliyunSubdomainUpdated(String recordId) {
        this.subdomainRecordId = recordId;
    }

    public void onAliyunSubdomainDeleted() {
        this.subdomainPrefix = null;
        this.subdomainReady = false;
        this.subdomainRecordId = null;
    }

    public void forceClearSubdomain(User user) {
        raiseEvent(new TenantSubdomainUpdatedEvent(this.getId(), this.subdomainPrefix, null, user));
        this.subdomainPrefix = null;
        this.subdomainReady = false;
        this.subdomainUpdatedAt = null;
        addOpsLog("清空子域名", user);
    }

    public void updateSubdomainReadyStatus(boolean ready, User user) {
        boolean result = ready && isNotBlank(subdomainPrefix);

        if (result != this.subdomainReady) {
            this.subdomainReady = result;
            raiseEvent(new TenantSubdomainReadyStatusUpdatedEvent(this.getId(), result, user));
            addOpsLog("设置子域名就绪状态:" + ready, user);
        }
    }
