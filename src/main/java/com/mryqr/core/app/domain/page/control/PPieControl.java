...
@Getter
@SuperBuilder
@TypeAlias("PIE_CONTROL")
@EqualsAndHashCode(callSuper = true)
@NoArgsConstructor(access = PRIVATE)
public class PPieControl extends Control {
    ...
    @NotNull
    private StatRange range;//统计时间范围
    ...
    protected void doValidate(AppSettingContext context) {
        ...
        if (context.pageNotExists(pageId)) {
            throw new MryException(VALIDATION_PAGE_NOT_EXIST, "饼状图引用的页面不存在。", mapOf("refPageId", pageId));
        }

        if (context.controlNotExists(pageId, basedControlId)) {
            throw new MryException(VALIDATION_CONTROL_NOT_EXIST, "饼状图引用的基准控件不存在。",
                    mapOf("refPageId", pageId, "refBasedControlId", basedControlId));
        }

        if (!context.controlTypeOf(basedControlId).isAnswerCategorized()) {
            throw new MryException(NOT_SUPPORTED_BASED_CONTROL_FOR_PIE, "基准控件不支持饼状图。",
                    mapOf("refPageId", pageId, "refBasedControlId", basedControlId));
        }

        if (segmentType != SUBMIT_COUNT_SUM) {
            if (context.controlNotExists(pageId, targetControlId)) {
                throw new MryException(VALIDATION_CONTROL_NOT_EXIST, "饼状图引用的目标控件不存在。",
            ...
            if (!valueControlType.isAnswerNumbered()) {
                throw new MryException(NOT_SUPPORTED_TARGET_CONTROL_FOR_PIE, "目标控件必须为数字输入类型。",
                        mapOf("refPageId", pageId, "refTargetControlId", targetControlId));
            }
        }
    }
