...
public class MongoDepartmentRepository extends MongoBaseRepository<Department> implements DepartmentRepository {
    ...
    public List<TenantCachedDepartment> cachedTenantAllDepartments(String tenantId) {
        ...
        return cachedDepartmentRepository.cachedTenantAllDepartments(tenantId).getDepartments();
    }
    ...
    public boolean cachedNotAllDepartmentsExist(List<String> departmentIds, String tenantId) {
        ...
        return !cachedDepartmentRepository.cachedTenantAllDepartments(tenantId).getDepartments().stream()
                .map(TenantCachedDepartment::getId).collect(toSet())
    ...
    public void delete(List<Department> departments) {
        ...
        departments.stream().findAny()
                .ifPresent(department -> cachedDepartmentRepository.evictTenantDepartmentsCache(department.getTenantId()));
    }
