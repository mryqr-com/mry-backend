    ...
    private void createAssignment(AssignmentPlan assignmentPlan, LocalDateTime startTime) {
        ...
        allVisibleGroupIds.forEach(groupId -> {
            try {
                ...
                Assignment assignment = assignmentFactory.create(groupId, qrIds, startTime, assignmentPlan, NO_USER);
                assignmentRepository.save(assignment);

                log.info("Created assignment[{}] for group[{}] from assignment plan[{}] of tenant[{}].",
                        assignment.getId(), groupId, assignmentPlan.getId(), assignmentPlan.getTenantId());
            } catch (Throwable t) {
