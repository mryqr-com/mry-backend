...
public class Assignment extends AggregateRoot {
    ...
    private AssignmentStatus deriveStatus(User user) {
        if (isClosed()) {
            return this.status;
        }
        ...
        if (isAllQrFinished()) {
            if (this.status != SUCCEED) {
                raiseEvent(new AssignmentFinishedEvent(this.getId(), user));
            }
            return SUCCEED;
        }
        ...
        if (isExpired()) {
            if (this.status != FAILED) {
                raiseEvent(new AssignmentFailedEvent(this.getId(), user));
            }
            return FAILED;
        }
        ...
        if (isNearExpire()) {
            if (this.status == IN_PROGRESS) {
                raiseEvent(new AssignmentNearExpiredEvent(this.getId(), user));
            }
            return NEAR_EXPIRE;
        }
