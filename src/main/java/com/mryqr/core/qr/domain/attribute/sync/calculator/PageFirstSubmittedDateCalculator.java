...
public class PageFirstSubmittedDateCalculator implements AttributeValueCalculator {
    private final SubmissionRepository submissionRepository;

    @Override
    public boolean canCalculate(Attribute attribute, App app) {
        return attribute.getType() == PAGE_FIRST_SUBMITTED_DATE;
    }
    ...
    public AttributeValue calculate(Attribute attribute, QR qr, App app) {
        return submissionRepository.firstInstanceSubmission(qr.getId(), attribute.getPageId())
                .map(submission -> new LocalDateAttributeValue(attribute, ofInstant(submission.getCreatedAt(), systemDefault()).toString()))
                .orElse(null);
    }
}
