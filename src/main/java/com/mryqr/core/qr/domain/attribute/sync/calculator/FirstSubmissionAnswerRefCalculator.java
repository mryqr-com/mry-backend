...
public class FirstSubmissionAnswerRefCalculator implements AttributeValueCalculator {
    private final SubmissionRepository submissionRepository;

    @Override
    public boolean canCalculate(Attribute attribute, App app) {
        return attribute.getType() == CONTROL_FIRST;
    }

    @Override
    public AttributeValue calculate(Attribute attribute, QR qr, App app) {
        return app.controlByIdOptional(attribute.getControlId())
                .flatMap(control -> submissionRepository.firstInstanceSubmission(qr.getId(), attribute.getPageId())
