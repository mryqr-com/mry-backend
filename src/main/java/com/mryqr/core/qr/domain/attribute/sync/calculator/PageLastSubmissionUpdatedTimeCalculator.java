...
public class PageLastSubmissionUpdatedTimeCalculator implements AttributeValueCalculator {
    private final SubmissionRepository submissionRepository;

    @Override
    public boolean canCalculate(Attribute attribute, App app) {
        return attribute.getType() == PAGE_LAST_SUBMISSION_UPDATED_TIME;
    }
    ...
    public AttributeValue calculate(Attribute attribute, QR qr, App app) {
        return submissionRepository.lastInstanceSubmission(qr.getId(), attribute.getPageId())
                .map(submission -> new TimestampAttributeValue(attribute, submission.getUpdatedAt()))
                .orElse(null);
    }
}
