...
public class SubmissionCategorizedAnswerSegmentReporter {
    private final MongoTemplate mongoTemplate;

    public List<List<CategorizedOptionSegment>> reportForQrMultiple(String qrId,
        ...
        if (segmentType == SUBMIT_COUNT_SUM) {
            return List.of(reportForQr(qrId, segmentType, pageId, basedControlId, null, range, app));
        }

        return targetControlIds.stream()
                .map(targetControlId -> reportForQr(qrId, segmentType, pageId, basedControlId, targetControlId, range, app))
                .collect(toImmutableList());
    }

    public List<CategorizedOptionSegment> reportForQr(String qrId,
                                                      SubmissionSegmentType segmentType,
                                                      String pageId,
                                                      String basedControlId,
        ...
        return this.report(qrId, null, segmentType, pageId, basedControlId, targetControlId, range, app);
    }

    public List<List<CategorizedOptionSegment>> reportForGroupsMultiple(Set<String> groupIds,
                                                                        SubmissionSegmentType segmentType,
                                                                        String pageId,
                                                                        String basedControlId,
                                                                        List<String> targetControlIds,
                                                                        ...
                                                                        App app) {
        if (segmentType == SUBMIT_COUNT_SUM) {
            return List.of(reportForGroups(groupIds, segmentType, pageId, basedControlId, null, range, app));
        }

        return targetControlIds.stream()
                .map(targetControlId -> reportForGroups(groupIds, segmentType, pageId, basedControlId, targetControlId, range, app))
                .collect(toImmutableList());
    }

    public List<CategorizedOptionSegment> reportForGroups(Set<String> groupIds,
                                                          SubmissionSegmentType segmentType,
                                                          String pageId,
                                                          String basedControlId,
                                                          String targetControlId,
        ...
        requireNonNull(range, "Statistic range must not be null.");
        requireNonNull(app, "App must not be null.");
        return this.report(null, groupIds, segmentType, pageId, basedControlId, targetControlId, range, app);
    }

    private List<CategorizedOptionSegment> report(String qrId,
                                                  Set<String> groupIds,
                                                  SubmissionSegmentType segmentType,
                                                  String pageId,
                                                  String basedControlId,
